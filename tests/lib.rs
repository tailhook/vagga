...
pub fn check_status_output_re(cmd: Command, status: int,
                           stdout: &Regex, stderr: &Regex)
{
    let out = cmd.spawn().unwrap().wait_with_output().unwrap();
    let mut err = false;
    if out.status != ExitStatus(status) {
    ...
    if !stdout.is_match(from_utf8(out.output.as_slice()).unwrap()) {
        println!("Command {} errorneous stdout:\n{}", cmd,
            from_utf8(out.output.as_slice()));
        err = true;
    }
    if !stderr.is_match(from_utf8(out.error.as_slice()).unwrap()) {
        println!("Command {} errorneous stderr:\n{}", cmd,
            from_utf8(out.error.as_slice()));
        err = true;
    }
